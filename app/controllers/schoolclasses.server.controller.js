 ...
 */
exports.list = function(req, res) { Schoolclass.find().sort('-created').populate('user', 'displayName').exec(function(err, schoolclasses) {
		if (err) {
			return res.send(400, {
				message: getErrorMessage(err)
			});
 ...
 */
exports.schoolclassByID = function(req, res, next, id) { Schoolclass.findById(id).populate('user', 'displayName').exec(function(err, schoolclass) {
